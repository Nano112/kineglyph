...
- `buildSurface({ focus, dimensions, ghost })` — click-to-focus
  (sheet node id → anchor, on clicks on the sheet), and dimension lines between
  anchors drawn in the view, and an optional faint preview of blocks still to come (off by
  default); the frame signals add `current` (the group in progress) and `dimension.<from>.<to>`.
